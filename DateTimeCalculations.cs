...
namespace APRCalculator
{
    public class DateTimeCalculations
    {
        ...
        internal static double PeriodsPerYear(UnitPeriod period)
        {
            switch (period.periodType)
            {
                case UnitPeriodType.Monthly:
                    return Convert.ToDouble(12d / period.numPeriods);
                case UnitPeriodType.Weekly:
                    return Convert.ToDouble(52d / period.numPeriods);
                case UnitPeriodType.Yearly:
                    return 1d; //The maximum UnitPeriod is 1 year, so this is always 1 (as opposed to say .5 for a 2 year common period)
                case UnitPeriodType.Daily:
                    return Convert.ToDouble(365d / period.numPeriods);
                default:
                    throw new ApplicationException("Invalid Unit Period Type passed into DaysPerPeriod");
            }
        }
        ...
        internal static int DaysPerPeriod(UnitPeriod period)
        {
            ...
            {
                case UnitPeriodType.Monthly:
                    return 30 * period.numPeriods;
                case UnitPeriodType.Weekly:
                    return 7 * period.numPeriods;
                case UnitPeriodType.Yearly:
                    return 365 * period.numPeriods;
                case UnitPeriodType.Daily:
        ...
        internal static DateTime AddPeriodToDate(DateTime currDate, UnitPeriod period)
        {
            ...
            {
                case UnitPeriodType.Monthly :
                    return currDate.AddMonths(period.numPeriods);
                case UnitPeriodType.Weekly :
                    return currDate.AddDays(period.numPeriods * 7);
                case UnitPeriodType.Daily :
                    return currDate.AddDays(period.numPeriods);
                case UnitPeriodType.Yearly :
                    return currDate.AddYears(period.numPeriods);
                default :
                    throw new ApplicationException("Invalid Period Type passed in");
            }
        }
        ...
        internal static string getUnitPeriodString(UnitPeriod period)
        {
            ...
            {
                case UnitPeriodType.Daily:
                    return period.numPeriods.ToString() + "D";
                case UnitPeriodType.Weekly:
                    return period.numPeriods.ToString() + "W";
                case UnitPeriodType.Monthly:
                    return period.numPeriods.ToString() + "M";
                case UnitPeriodType.Yearly:
                    return period.numPeriods.ToString() + "Y";
                default:
                    throw new ApplicationException("Invalid Period Type in getUnitPeriodString");
            }
        }
